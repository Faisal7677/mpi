...
namespace TopologyAwareResearch {
    ...
    PerformanceMetrics TopologyAwareBroadcast::binomial_tree_broadcast(void* buffer, int count,
        ...
        // Advanced binomial tree with topology awareness
        int relative_rank = (world_rank - root + world_size) % world_size;
        int mask = 1;

        while (mask < world_size) {
            if (relative_rank < mask) {
                int dest_relative = relative_rank + mask;
                if (dest_relative < world_size) {
                    int dest = (root + dest_relative) % world_size;
    ...
    PerformanceMetrics TopologyAwareBroadcast::multi_core_broadcast(void* buffer, int count,
        ...
        MPI_Comm node_comm;
        int node_id = world_rank / network_config_.processes_per_node;
        ...
        int root_node = root / network_config_.processes_per_node;
        int root_node_rank = root % network_config_.processes_per_node;
        ...
        if (node_id == root_node) {
            if (node_rank == root_node_rank) {
                ...
                for (int n = 0; n < network_config_.total_nodes; n++) {
                    if (n != root_node) {
                        int node_leader = n * network_config_.processes_per_node;
    ...
    std::vector<int> TopologyAwareBroadcast::construct_k_ary_tree(int root, int world_size, int k) {
        ...
        while (!q.empty() && tree.size() < world_size) {
            int current = q.front();
            q.pop();

            for (int i = 1; i <= k && tree.size() < world_size; ++i) {
    ...
    int TopologyAwareBroadcast::get_node_id(int rank) const {
        if (rank < network_config_.node_mapping.size()) {
            return network_config_.node_mapping[rank];
        }
        return rank / network_config_.processes_per_node;
    }
    ...
    PerformanceMetrics HierarchicalAllreduce::three_level_allreduce(const void* sendbuf, void* recvbuf,
        ...
        void* node_result = (node_rank == 0) ? malloc(count * get_mpi_type_size(datatype)) : nullptr;
        ...
        if (rack_comm != MPI_COMM_NULL) {
            ...
            void* rack_result = (rack_rank == 0) ? malloc(count * get_mpi_type_size(datatype)) : nullptr;
            if (node_rank == 0) {
            ...
            if (global_comm != MPI_COMM_NULL) {
                ...
                void* global_result = (global_rank == 0) ? malloc(count * get_mpi_type_size(datatype)) : nullptr;
                if (rack_rank == 0) {
                ...
                if (global_rank == 0) {
                    memcpy(recvbuf, global_result, count * get_mpi_type_size(datatype));
                }
    ...
    PerformanceMetrics HierarchicalAllreduce::segmented_ring_allreduce(const void* sendbuf, void* recvbuf,
        ...
        std::vector<char> temp_buffer(count * get_mpi_type_size(datatype));
        ...
        for (int seg = 0; seg < segments; seg++) {
            ...
            for (int step = 0; step < size - 1; step++) {
                ...
                MPI_Send(reinterpret_cast<char*>(recvbuf) + seg_start * get_mpi_type_size(datatype),
            ...
            for (int step = 0; step < size - 1; step++) {
                ...
                MPI_Recv(reinterpret_cast<char*>(recvbuf) + seg_start * get_mpi_type_size(datatype),
        ...
        metrics.bytes_transferred = 2 * (size - 1) * count * get_mpi_type_size(datatype);
        metrics.communication_edges = communication_edges;
        metrics.messages_sent = communication_edges.size();
...
//// Implement create_rack_communicator
//MPI_Comm TopologyAwareResearch::HierarchicalAllreduce::create_rack_communicator(MPI_Comm comm) {
//    int rank;
//    MPI_Comm_rank(comm, &rank);
//
//    int rack_id = rank / 4;  // Assuming 4 nodes per rack
//    MPI_Comm rack_comm;
//    MPI_Comm_split(comm, rack_id, rank, &rack_comm);
//    return rack_comm;
//}
